...
def popular_articles():
    popular_article_data_list = []

    for index, row in output.iloc[:, 0:20].iterrows():
        _p = {
            "url" : row["url"],
            "title" : row["title"],
    ...
    popular_article_data_list.append(_p)
    
    return jsonify({
        "data" : popular_article_data_list,
        "status": "success",
    })
...
def recommended_articles(): 
    ...
    for index, row in all_recommended.iloc[:, 0:10].iterrows():
        _p = {
            "url" : row["url"],
            "title" : row["title"],
            "text" : row["text"] or "N/A",
